...
if __name__ == "__main__":
    config: KConfig = KConfig.from_file(path=pathlib.Path(".config"))
    ...
    config.write_to_file(out=pathlib.Path(".config2"))

    other_config: KConfig = KConfig.from_file(path=pathlib.Path(".config2"))
