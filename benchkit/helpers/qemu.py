...
def default_busybox() -> PathType:

    busybox_path: pathlib.Path = QEMU_ARTIFACTS / "busybox"
...
class Qemu(CommunicationLayer, Platform):
    def __init__(
            self,
            number_of_cpus: int,
            memory: int,
            ...
            utilities: Callable[[], PathType] = default_busybox, # function that returns to the stuff that should go in /bin 
            initrd: str = "",
        ...
        self._memory: int
        self._mounts: list[Mount] = mounts

        self._args: list[str] = list()
        if shared_dir is not None:
            # TODO if interested, abstract virtfs into a file system class node
            self._args.append(f"-virtfs local,path={shared_dir},mount_tag=host0,security_model=none")
            self._mounts.append(Mount(what=shared_dir, where="/host", _type="9p", args=["trans=virtio"]))

        try:
            if clean_build:
                shutil.rmtree(QEMU_ARTIFACTS)
            os.mkdir(QEMU_ARTIFACTS)
        except FileExistsError:
            os.remove(QEMU_ARTIFACTS / "initramfs.cpio.gz")
            os.remove(QEMU_ARTIFACTS / "init")
        except Exception:
            raise Exception("mkdir problem") # TODO make it less retarded

        self.initramfs = self._make_initramfs(utils=utilities())
        self._initrd: str = f"{self.initramfs}"




        # -virtfs local,path=./shared,mount_tag=host0,security_model=none   \
        # -serial pty \
    ...
    # NOTE we should probably find a better place for this
    def _make_initramfs(self, utils: PathType) -> PathType:

        initramfs_root: pathlib.Path = QEMU_ARTIFACTS / "initramfs"
        os.mkdir(initramfs_root)

        os.mkdir(initramfs_root / "bin")
        os.mkdir(initramfs_root / "sbin")
        os.mkdir(initramfs_root / "etc")
        os.mkdir(initramfs_root / "proc")
        os.mkdir(initramfs_root / "sys")
        os.mkdir(initramfs_root / "dev")
        os.mkdir(initramfs_root / "usr" / "bin")
        os.mkdir(initramfs_root / "usr" / "sbin")
        os.mkdir(initramfs_root / "usr" / "local" / "bin")

        for item in pathlib.Path(utils).iterdir():
            target = initramfs_root / item.name
            if item.is_dir():
        ...
        return QEMU_ARTIFACTS / compressed_initramfs
...
# 
# mkdir -p /host
# mount -t 9p host0 /host -o trans=virtio
# 
# exec /bin/sh
    ...
    def shell(
        ...
        fd = os.open(self._pty, os.O_RDWR | os.O_NOCTTY)
        ...
        if current_dir is not None:
            os.write(fd, " ".join(["cd", str(current_dir), "\n"]).encode("utf-8"))
